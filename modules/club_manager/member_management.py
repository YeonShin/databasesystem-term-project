...
def add_member_to_club(mydb, club_id):
  ...
  # 추가할 부원의 학번 입력받기
  student_id = input("동아리에 등록할 학생의 학번을 입력하세요: ").strip()
  ...
  if student['Club_id'] == club_id:
    ...
    os.system('clear')
    print(f"해당 학생 (학번: {student['Student_id']}, 이름: {student['Sname']})은 이미 다른 동아리 (ID: {student['Club_id']})에 소속되어 있습니다.")
  else:
    ...
    WHERE Student_id = %s
    """
    cursor.execute(query, (club_id, student_id))
    mydb.commit()
    os.system('clear')
...
# (동아리장 기능) 내 동아리 부원 전체 조회 함수
def select_all_members(mydb, club_id):
  cursor = mydb.cursor(dictionary=True)
...
def remove_member_from_club(mydb, club_id):
  ...
  if member['Role'] == '동아리장':
    os.system('clear')
    print('동아리장 직책을 가진 부원은 탈퇴할 수 없습니다.')
    cursor.close()
    return
